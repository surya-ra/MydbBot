...
.matches('ServiceRequest',[
  function(session,args){
    ...
    var entityCount=builder.EntityRecognizer.findEntity(args.entities,'ServiceRequest Count');
    var entityCompliance=builder.EntityRecognizer.findEntity(args.entities,'ServiceRequest Compliance');
    ...
    //debugging purpose
    console.log('ServiceRequest Count '+JSON.stringify(entityCount));
    console.log('ServiceRequest Compliance '+JSON.stringify(entityCompliance));
    ...
    //Base JSON: ServiceRequestCount
    //Top JSON : TopServiceRequestCount

    if ((JSON.stringify(entityCount)!='null')&&(JSON.stringify(entityCompliance)=='null')) {           
      for(var i=0;i<BaseQueryContents.length;++i){
        if (BaseQueryContents[i].intent=="ServiceRequestCount") {
          BasePassQueryAssetCount=BaseQueryContents[i].query;
        }
      }
      for (var i=0;i<TopQueryContents.length;++i) {
       	if (TopQueryContents[i].intent=="TopServiceRequestCount") {
...
/*1*/ if ((JSON.stringify(entityTop)!='null')&&(JSON.stringify(entityTopDept)=='null')&&(JSON.stringify(entityTopFacility)=='null')&&(JSON.stringify(entityTopType)=='null')&&(JSON.stringify(entityTopYear)=='null')) {       
        ...
        global.requestCountTop= new Request(
          PassQuery,//query for count and top by SR category
...
/*2*/ else if ((JSON.stringify(entityTop)!='null')&&(JSON.stringify(entityTopDept)!='null')&&(JSON.stringify(entityTopFacility)=='null')&&(JSON.stringify(entityTopType)=='null')&&(JSON.stringify(entityTopYear)=='null')) {
        var PassQuery=BasePassQueryTop+BasePassQueryGroup;
        global.requestCountTop= new Request(
          PassQuery,//query for count and top by SR department
          function(err,rowCount,rows){
            if (err) {
              console.log(err);
            }
...
/*7*/ else if ((JSON.stringify(entityTop)=='null')&&(JSON.stringify(entityFilterYear)=='null')&&(JSON.stringify(entityFilterType)=='null')&&(JSON.stringify(entityFilterDept)=='null')&&(JSON.stringify(entityFilterCritical)=='null')&&(JSON.stringify(entityFilterNonCritical)=='null')) {                      //NO FILTER        
    ...
    /***AUTHOR:Suryadeep***/
    //logic for handling LUIS and bot response
    //Base JSON: ServiceRequestCompliance
    //Top JSON : TopServiceRequestCompliance
	  else if((JSON.stringify(entityCompliance)!='null')&&(JSON.stringify(entityCount)=='null')) {
      for(var i=0;i<BaseQueryContents.length;++i){
        if (BaseQueryContents[i].intent=="ServiceRequestCompliance") {
          BasePassQueryAssetValue=BaseQueryContents[i].query;
        }
      }
      for (var i=0;i<TopQueryContents.length;++i) {
       	if (TopQueryContents[i].intent=="TopServiceRequestCompliance") {
       		BasePassQueryTop=TopQueryContents[i].query;
       		BasePassQueryGroup=TopQueryContents[i].group;
       	}
      }
...
/*7*/ else if ((JSON.stringify(entityTop)=='null')&&(JSON.stringify(entityFilterYear)=='null')&&(JSON.stringify(entityFilterType)=='null')&&(JSON.stringify(entityFilterDept)=='null')&&(JSON.stringify(entityFilterCritical)=='null')&&(JSON.stringify(entityFilterNonCritical)=='null')) {                      //NO FILTER        
      ...
      console.log('SERVICE REQUEST COUNT: X');
      console.log('SERVICE REQUEST COMPLIANCE: X')
