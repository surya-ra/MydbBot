     ...
     // var botbuilder_azure = require("botbuilder-azure");
      ...
      var GetReplies= jsonFile.readFileSync("PostDeployScripts\\Replies.json")
...
function getentities(session,entity,column){
  ...
  /*    if(filterentity.length>0){
    	...
    	requestCountTop.on('row',function(columns){
          ...
          //session.send(Hyperlink);	
          StoreForTop[index_top]=column.value;
          index_top=index_top+1;	
    		});
    		sendTopValues(session,StoreForTop,index_top)
    	});
      
    	ConnectionTop.execSql(requestCountTop);
      ...
      for(var x=0;x<GetReplies.length;x++){
        if (GetReplies[x].Entities==PrimaryEntity) {
          console.log('reply is '+GetReplies[x].one);
          StoredReply= GetReplies[x].one;
        }
      }
    	var requestCountTop= new Request(
    		PassQuery,
      ...
      console.log('request : '+requestCountTop);
    	...
    	requestCountTop.on('row',function(columns){
    		columns.forEach(function(column){
    			session.send("%s:\t%s", StoredReply, column.value);
        /* builder.Prompts.number(session,"Would you like to see a graphical report " + '1. YES  2.NO',function(session){
...
function sendValues(session,args,args1){
  /*Update : 1.5.0*/
  console.log(args);
  console.log("Index before if: "+SlNo)
  SlNo=SlNo+1;
  session.send('%s--> %s',args1[args-2],args1[args-1])
}

function sendTopValues(session,args_Top,args1_Top){
  session.send("%s-->%s ", args_Top[args1_Top-2],args_Top[args1_Top-1])
}
